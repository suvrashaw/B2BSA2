          ...
          <div className="container mx-auto px-8 pt-32 pb-8 relative">
            <div className="grid lg:grid-cols-12 gap-16 mb-16">
              ...
              <div className="lg:col-span-5">
                <Link href="/" className="relative h-12 w-48 block mb-8 transition-opacity hover:opacity-90">
                  <Image
                    src="/logo.png"
                    alt="B2B Sales Arrow"
                    fill
                ...
                <p className="text-gray-600 leading-relaxed mb-8 max-w-sm">
                  The premier growth partner for ambitious B2B enterprises. We design, execute, and scale high-conversion physical and digital experiences globally.
                </p>
                
                <div className="flex items-center gap-4">
                  <a href="#" className="w-10 h-10 rounded-full bg-brand-gray border border-gray-200 flex items-center justify-center text-brand-blue hover:bg-brand-blue hover:text-white hover:border-brand-blue transition-all duration-300">
              ...
              <div className="lg:col-span-2">
                <h4 className="font-heading font-bold text-lg text-brand-charcoal mb-6">Navigation</h4>
                <ul className="space-y-4">
                  {footerNavigation.map((item) => (
                    <li key={item.name}>
                      <Link href={item.href} className="text-gray-500 hover:text-brand-blue transition-colors text-sm font-medium">
                        {item.name}
                      </Link>
                    </li>
                  ))}
              ...
              <div className="lg:col-span-3">
                <h4 className="font-heading font-bold text-lg text-brand-charcoal mb-6">Services</h4>
                <div className="grid gap-4">
                  {serviceNavigationGroups.map((group) => (
                    <div key={group.name}>
                      <Link href={group.href} className="text-sm font-bold text-gray-600 hover:text-brand-blue transition-colors">
                        {group.name}
                      </Link>
                      {group.links.length > 0 && (
                        <ul className="mt-2 space-y-2 pl-3 border-l border-gray-200">
                          {group.links.map((item) => (
                            <li key={item.name}>
                              <Link href={item.href} className="text-xs leading-5 text-gray-400 hover:text-brand-blue transition-colors">
                                {item.name}
                              </Link>
                            </li>
                          ))}
              ...
              <div className="lg:col-span-2">
                <h4 className="font-heading font-bold text-lg text-brand-charcoal mb-6">Stay Ahead</h4>
                <p className="text-sm text-gray-500 mb-4">Subscribe to our enterprise growth newsletter.</p>
            ...
            <div className="pt-8 border-t border-gray-200 flex flex-col md:flex-row items-center justify-between gap-4 pointer-events-auto">
              ...
              <div className="flex gap-6 text-xs text-gray-500">
                ...
                <Link href="/terms-and-conditions" className="hover:text-brand-blue transition-colors">Terms &amp; Conditions</Link>
                <Link href="#" className="hover:text-brand-blue transition-colors">Cookie Settings</Link>
              </div>
            </div>
